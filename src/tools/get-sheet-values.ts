...
export class GetSheetValuesTool implements IMCPTool {
  ...
  readonly parameters = {
    ...
    range: z.string().optional().describe("Optional cell range in A1 notation (e.g. A1:D5)"),
  };
   ...
   */
  private formatSheetValues(values: any[][], sheetName: string, range?: string): string {
    ...
    for (let row = 0; row < rowCount; row++) {
      result += '|';
      for (let col = 0; col < colCount; col++) {
        const cellValue = values[row][col] !== undefined ? String(values[row][col]) : '';
        // セル値が長い場合は省略
        const formattedValue = cellValue.length > colWidths[col]
